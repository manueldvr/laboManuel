################################################################################
##
##  / FeatureEngineering / 601_FE_historico
##
################################################################################
...
################################################################################
kdataset_salida  <- "paquete_premium_ext_002.csv.gz"
# aca se define los campos fijos???
...
AgregarVariables  <- function( dataset )
{
  ...
  dataset[ , n_active_q_int          := active_quarter  +  internet ]
  dataset[ , n_active_q_mren         := active_quarter  +  mrentabilidad ]
  dataset[ , n_active_qmcuentas      := active_quarter  +  mcuentas_saldo ]
  dataset[ , n_active_vistrx_cons    := mv_mpagominimo  + ctarjeta_visa_trx	+ mtarjeta_visa_consumo	 ]  #Cantidad de transacciones efectuadas durante el mes con la tarjeta de crédito VISA	
                                                                                                        #Monto total de los consumos efectuados durante el mes con la tarjeta de crédito VISA	
  dataset[ , n_active_mastrx_coms    := mv_mpagominimo  + ctarjeta_master_trx	+ mtarjeta_master_consumo ]
  dataset[ , n_prestamos             := mprestamos_prendarios	+ cplazo_fijo	+ mplazo_fijo_pesos	 ]
